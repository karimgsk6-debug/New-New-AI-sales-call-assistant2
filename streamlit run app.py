...
# --- Press-and-hold style recording ---
st.subheader("🎤 Press and Hold to Record (Release to Send)")
if st.button("Start Recording"):
    st.session_state.recording = True
    webrtc_ctx = webrtc_streamer(
        key="voice", mode=WebRtcMode.SENDRECV,
        audio_receiver_size=1024, media_stream_constraints={"audio": True, "video": False}
    )

if st.session_state.recording and st.button("Stop & Send"):
    st.session_state.recording = False
    if webrtc_ctx.audio_receiver:
        ...
        if frames:
            ...
            try:
                ...
                st.session_state.rep_voice_text = "❌ Could not transcribe audio."
    st.text_area("Your message:", value=st.session_state.rep_voice_text, height=100)

# --- Chat submission ---
with st.form("chat_form", clear_on_submit=True):
    user_input = st.text_area("Type your message (or use voice above)", value=st.session_state.rep_voice_text)
