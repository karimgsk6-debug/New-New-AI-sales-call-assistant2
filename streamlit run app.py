...
def generate_prompt_suggestions():
    suggestions = [
        f"Generate call flow for {persona} in segment {segment}",
        f"Draft key discussion points for {specialty} on {brand}",
        f"Handle objection: {barrier[0] if barrier else 'No barrier'}",
        f"Summarize uploaded PDF into actionable bullets",
        f"Create persuasive message with {response_tone} tone"
    ]
    st.session_state.prefilled_prompt = st.selectbox("💡 Prompt Suggestions (Copilot)", suggestions, key="copilot_suggestion")
