...
# ----------------------------
BACKGROUND_URL = ("https://sdmntprnortheu.oaiusercontent.com/files/00000000-7268-61f4-9aa6-71a39056c20e/raw?se=2025-09-25T15%3A42%3A47Z&sp=r&sv=2024-08-04&sr=b&scid=dfa0d35f-01ac-5224-bec7-ff9f505758dd&skoid=b32d65cd-c8f1-46fb-90df-c208671889d4&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skt=2025-09-25T09%3A41%3A15Z&ske=2025-09-26T09%3A41%3A15Z&sks=b&skv=2024-08-04&sig=ap%2BO7ty9YJurxH528T8cPoSQD5Kh6VHdsvf%2Fnvdkbjs%3D")
...
# ----------------------------
st.markdown("""<div style="position:fixed; top:72px; left:18px; z-index:1000; background: rgba(255,255,255,0.9); padding:8px 12px; border-radius:8px;">""", unsafe_allow_html=True)
...
gsk_brands_images = {"Shingrix":"https://www.oma-apteekki.fi/WebRoot/NA/Shops/na/67D6/48DA/D0B0/D959/ECAF/0A3C/0E02/D573/3ad67c4e-e1fb-4476-a8a0-873423d8db42_3Dimage.png","Trelegy":"https://www.1uphealth.com/wp-content/uploads/2020/11/trelegy.png","Zejula":"https://cdn.salla.sa/QeZox/eyy7B0bg8D7a0Wwcov6UshWFc04R6H8qIgbfFq8u.png"}
race_segments = ["R – Reach: Not prescribing yet; doesn't see vaccination responsibility.","A – Acquisition: Prescribes when patient asks; convinced by data.","C – Conversion: Initiates for specific profiles; not across all profiles.","E – Engagement: Proactively prescribes across multiple patient profiles."]
...
sales_call_flow = ["Prepare: Data & patient profiles","Engage: Opening question & rapport","Create Opportunities: Identify eligible patients","Influence: Present tailored evidence & handle objections","Drive Impact: Secure next steps (prescription/scheduling)","Post Call Analysis: Document & follow up"]
APACT_STEPS = ["Acknowledge","Probing","Action","Confirm","Transition"]
objectives = ["Awareness","Adoption","Retention"]
specialties = ["GP","Cardiologist","Dermatologist","Endocrinologist","Pulmonologist"]

# ----------------------------
# Sidebar filters (brand image below selector)
# ----------------------------
    ...
    if uploaded_pdf:
        try:
            ...
            if st.button("Summarize PDF"):
                ...
                summary_prompt = (f"Summarize this medical document for sales reps for {brand}. Focus on actionable findings, "
...
def synthesize_tts_base64(text, lang):
    ...
    tmp.close()
    try:
        async def _save(): await edge_tts.Communicate(text, voice=voice).save(tmp_name)
        asyncio.run(_save())
        with open(tmp_name,"rb") as f: audio_bytes = f.read()
...
if submitted and user_input.strip():
    ...
    try:
        resp = client.chat.completions.create(
            model="meta-llama/llama-4-scout-17b-16e-instruct",
            messages=[{"role":"system","content":f"You are a helpful sales assistant that responds in {language}."},{"role":"user","content":prompt}],
            temperature=0.7
        )
