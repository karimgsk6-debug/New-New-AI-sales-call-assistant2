# app.py - AI Sales Call Assistant (Full merged with dynamic follow-up)
...
# Initialize your GROQ client here if available
...
.header {{display:flex; align-items:center; justify-content:space-between; gap:12px; background: linear-gradient(90deg, rgba(255,255,255,0.85), rgba(255,255,255,0.78)); padding:10px 18px; border-radius:10px; margin-bottom:12px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);}}
...
.chat-container {{max-height:56vh; overflow-y:auto; padding:12px; background: rgba(255,255,255,0.95); border-radius:8px; margin-bottom:160px; box-shadow:0 6px 18px rgba(11,22,55,0.04);}}
...
def export_call_flow_bytes(text:str,fmt:str="docx"):
    ...
    if fmt=="docx" and DOCX_AVAILABLE:
        ...
        b=text.encode("utf-8"); filename=f"{safe_brand}_callflow_{now}.txt"
        return b, filename
...
<div class="header">
  ...
  <div class="title"><h1 style="margin:0">💡 AI Sales Call Assistant</h1>
  <div style="color:#666;font-size:14px;">APACT-guided — {brand_data[st.session_state['selected_brand']]['display']}</div></div>
  <div style="width:140px;text-align:right;"><img src="{AI_LOGO_RAW}" class="right-logo"></div>
</div>
""",unsafe_allow_html=True)

# -------------------------
# Load brand summaries & corpus
# -------------------------
...
def add_ai_response(prompt:str,follow_up:bool=False,context_previous:str=None):
    ...
    out_lines.append("- 'I appreciate you bringing this up — it's an important point.'")
    out_lines.append("- 'That's a fair question. Let me walk you through what we've seen.'\n")
    if not follow_up:
        out_lines.append("**🟢 Acknowledge**\n- Show empathy and understanding.")
        out_lines.append("**💡 Educate**\n- Key facts about the brand/product.")
        out_lines.append("**📌 Close**\n- Call-to-action / next step\n")
    if snippets:
        out_lines.append("**Reference Snippets:**")
        for sn in snippets:
            out_lines.append(f"- {sn['text'][:180]}... (source: {sn['meta']['filename']})")
    out_text="\n".join(out_lines)
    st.session_state["chat_history"].append({"role":"ai","text":out_text})
...
with st.container():
    ...
    for idx, entry in enumerate(st.session_state["chat_history"]):
        ...
        if role=="user":
            st.markdown(f'<div class="chat-bubble-user">{escape(content)}</div>',unsafe_allow_html=True)
        else:
            ...
            if fcol1.button("👍", key=f"like_{idx}"):
                st.session_state["feedback_stats"]["like"] +=1

            if fcol2.button("👎", key=f"dislike_{idx}"):
                st.session_state["feedback_stats"]["dislike"] +=1
                # Generate dynamic follow-ups
                followups=[
                    "Make it shorter & punchy",
                    "Focus on objections handling",
                    "Add more persuasive language"
                ]
                st.session_state.setdefault("followup_options", {})[followup_key]=followups

            if fcol3.button("🔄 More", key=f"more_{idx}"):
                st.session_state["feedback_stats"]["need_more"] +=1
                add_ai_response(entry["text"]+" (Please elaborate in more detail)")

            # Display follow-ups if available
            if st.session_state.get("followup_options", {}).get(followup_key):
                st.markdown("💡 Select a follow-up to improve this response:")
                ...
                for i, q in enumerate(st.session_state["followup_options"][followup_key]):
                    if fcols[i].button(q.strip(), key=f"{followup_key}_opt_{i}"):
                        add_ai_response(entry["text"] + " User wants clarification: " + q.strip(), follow_up=True)
                        st.session_state["followup_options"].pop(followup_key)
                        st.experimental_rerun()
...
user_input=st.text_area("Ask the AI:", value="", key="main_input", height=120, placeholder="Type your question or request...")
cols=st.columns([1,1])
...
# -------------------------
# Prompt suggestions as pills
# -------------------------
st.markdown("<div style='margin-top:140px;'><b>Prompt Suggestions:</b></div>",unsafe_allow_html=True)
...
for i, s in enumerate(suggestions):
    if pill_cols[i].button(s, key=f"sugg_{i}"):
...
# Fixed disclaimer
# -------------------------
st.markdown('<div class="fixed-disclaimer">📌 Disclaimer: This AI tool is a sales assistant reference. Always follow local regulations and medical guidelines.</div>',unsafe_allow_html=True)
