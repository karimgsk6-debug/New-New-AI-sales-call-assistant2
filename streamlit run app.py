...
with st.sidebar.expander("Filters & Options", expanded=True):
    ...
    if img_path:
        try: st.image(Image.open(BytesIO(requests.get(img_path).content)), width=160)
    ...
    barrier = st.multiselect("Select Doctor Barrier / اختر حاجز الطبيب", doctor_barriers, default=[])
    ...
    st.session_state.language = st.radio("Language / اختر اللغة", ["English","العربية"], index=0, horizontal=True)
...
if uploaded_pdf:
    try:
        ...
        st.success("✅ PDF processed (text + references)")
    except Exception as e: st.error("PDF error: "+str(e))

# ----------------------------
# Helper functions
def highlight_content_for_display(content):
...
def build_ai_bubble_content(ai_text, inject_pdf_lines=6):
...
def render_chat_history():
    ...
    for msg in st.session_state.chat_history:
        ...
        if msg["role"]=="user": html_out+=f"<div class='chat-bubble-user'>{content}</div>"
        else:
